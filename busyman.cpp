...
int main() {
    std::cout<< "Cuantas actividades posibles hay?" << std::endl;
    cin >> N;    //N es la cantidad de actividades que tendremos, i la hora de inicio y j la hora de fin
    while (cont < N){
        std::cout<< "Ingresa el horario de inicio y luego de finalizacion" << std::endl;
        cin >> i >> j;
    ...
    if (N > 0){
       int aDevo = fRecursiva(0,0,actividades,MayoresActividades);
       std::cout<< "La cantidad maxima de actividades posibles es " << aDevo << std::endl;
       return aDevo;
    }
    else {
        std::cout<< " La cantidad es " << N << std::endl; 
        return 0;
    }
}
