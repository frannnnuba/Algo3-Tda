...
int fRecursiva(int indice, int W, int B,const vector<int>& secAP,vector<std::vector<std::vector<int>>>& tablaSoluciones){
    ...
    if(secAP[indice] > laBlanca && secAP[indice] < laNegra){
        laMejor = max(fRecursiva(indice +1,indice, B, secAP,tablaSoluciones) +1,max( 
                        fRecursiva((indice +1),W, indice,secAP,tablaSoluciones) +1,
                        fRecursiva(indice +1,W , B,secAP,tablaSoluciones))); 
    }
    if (secAP[indice] > laBlanca && !(secAP[indice] < laNegra)){
        laMejor = max(fRecursiva(indice +1,indice, B, secAP,tablaSoluciones) +1, 
                        fRecursiva(indice +1,W , B,secAP,tablaSoluciones));
    }
    if(secAP[indice] < laNegra && !(secAP[indice] > laBlanca)){
        laMejor = max(fRecursiva(indice +1,W, indice, secAP,tablaSoluciones) +1, 
        fRecursiva(indice +1,W , B,secAP,tablaSoluciones));
    }
    tablaSoluciones[indice][W+1][B] = laMejor;
    return laMejor;
}
